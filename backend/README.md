...
1. Add these Railway variables:
   ...
   - `SMS_PROVIDER=autocall`.
   - `AUTOCALL_API_TOKEN` - the AutoCall Bearer token. Never add it to GitHub or Vercel.
   - `AUTOCALL_API_BASE_URL=https://autocall.kz/api/v1`.
   - To roll back, use `SMS_PROVIDER=mobizon`, `MOBIZON_API_KEY`, and the optional `MOBIZON_SENDER`.
...
sending it to the selected provider. Only Kazakhstan E.164 numbers (`+7` plus 10 digits) are accepted.
