  ...
  if (submittedId) {
    ...
    return (
      <section className="sell sell-success">
        ...
        <p className="eyebrow">{published ? "Объявление принято" : "Черновик принят"}</p>
        <h1>{published ? "Загрузка продолжится в фоне" : "Можно продолжить позже"}</h1>
        <p>
          {published
            ? "Можно покинуть этот экран. После загрузки файлов автоматическая проверка сама опубликует объявление. Если полностью закрыть приложение раньше, загрузка возобновится при следующем запуске."
            : "Данные приняты. Фотографии и видео продолжат загружаться в фоне."}
        </p>
